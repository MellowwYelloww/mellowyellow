...
public class HUDmanager {
	...
	public HUDmanager(World world) {
		this.world = world;
		lightp = loadImageFrom.LoadImageFrom(Main.class, "light.png");
		addLights();
	}
	...
	private void addLights() {
			lights.add(new Light(600, 200, 300, 200));
	...
	public void render(Graphics2D g){
		...
		//g.drawImage(lightp, 0, 0, Main.width, Main.height,null);
		...
		if(world.getPlayer().isDebuging()){
			...
			g.drawString("[Current Loaded Blocks]"+world.getWorldBlocks().getLoadedBlocks().size() , 30, 210);
